...
def AddOrder(orderType, ticketer, quant, price):
    ...
    with book.lock:
        if orderType == 'Buy':
            book.buy_orders.append(order)
            print(f"Added Buy order: {quant} shares of {ticketer} at ${price}")
        else:
            book.sell_orders.append(order)
...
def matchOrder(ticketer):
    ...
    with book.lock:  # Lock to ensure thread-safety
        sellOrder = book.sell_orders
        buyOrder = book.buy_orders
        ...
        while i >= 0:
            if i >= len(buyOrder):
                i -= 1
                continue
            buy_order = buyOrder[i]
            sell_order_min_price_ind = -1
            sell_order_min_price = float('inf')

            for j in range(len(sellOrder)):
                sell_order = sellOrder[j]
                if sell_order.price < sell_order_min_price:
                    sell_order_min_price = sell_order.price
                    sell_order_min_price_ind = j

            if sell_order_min_price_ind == -1:
                i -= 1
                continue

            sell_order = sellOrder[sell_order_min_price_ind]
            if buy_order.price >= sell_order.price:
                ...
                if sell_order.quant == 0:
                    ...
                    sellOrder.pop(sell_order_min_price_ind)
